            ...
            <div className="bg-[#0b141b] w-full max-w-md border border-zinc-800 rounded-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="p-8 space-y-6">
                    ...
                    <div className="relative group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-600 group-focus-within:text-[#FACC15] transition-colors" />
                        <input
                            type="text"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            placeholder="Search by name or email..."
                            className="w-full bg-zinc-900/50 border border-zinc-800 rounded-md pl-12 pr-4 py-4 text-sm font-bold text-white placeholder:text-zinc-700 outline-none focus:border-[#FACC15]/30 focus:ring-4 focus:ring-[#FACC15]/5 transition-all"
                        />
                    </div>

                    {/* User List */}
                    <div className="max-h-[400px] overflow-y-auto space-y-2 pr-2 stealth-scrollbar">
                        {allUsers === undefined ? (
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between p-4 rounded-md transition-all ${isSelected
                                            ? 'bg-[#FACC15]/10 border border-[#FACC15]/20 shadow-lg shadow-[#FACC15]/5'
                                    ...
                                    >
                                        <div className="flex items-center gap-4 text-left min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <p className={`text-sm font-black truncate ${isSelected ? 'text-[#FACC15]' : 'text-zinc-200'}`}>{user.name}</p>
                                            </div>
                                        </div>
                                        <div className={`w-6 h-6 rounded-md border shrink-0 flex items-center justify-center transition-all ${isSelected ? 'bg-[#FACC15] border-[#FACC15]' : 'border-zinc-800 bg-zinc-900/50'
                                            }`}>
                                            {isSelected && <Check className="w-3 h-3 text-[#111827] stroke-[3]" />}
                                        </div>
                                    </button>
                                );
                            })
